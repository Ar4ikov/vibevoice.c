...
static void clamp_params(vv_frontend_params_t* p) {
    ...
    if (p->gather_us < 0) p->gather_us = 0;
}
...
static vv_status_t launch(vv_frontend_t* fe, fe_piece_t* pc, int np) {
    ...
    s = vv_dev_event_sync(fe->ev_pin[fe->pin_i]);
    if (s != VV_OK) return s;
    int64_t total = 0;
    ...
    for (int i = 0; i < np; i++) {
        poff[i] = total;
        if (pc[i].len > 0)
            memcpy(pin + total, pc[i].job->audio + pc[i].off,
    ...
    if (n_rows > fe->f_max) return VV_ERR_OVERFLOW;
...
static void finish_job(vv_frontend_t* fe, vv_frontend_job_t* j, fe_prog_t* pg,
                       vv_status_t s) {
    pg->finished = true;
    j->status = s;
    j->n_frames = pg->written;
...
static vv_status_t run_ptrs(vv_frontend_t* fe, vv_frontend_job_t** jobs, int n) {
    if (n <= 0) return VV_OK;
    fe_prog_t stack_pg[VV_VAE_MAX_ITEMS];
    fe_prog_t* pg = n <= VV_VAE_MAX_ITEMS ? stack_pg
                  : (fe_prog_t*)vv_alloc((size_t)n * sizeof(fe_prog_t));
    if (!pg) return VV_ERR_OUT_OF_MEMORY;
    memset(pg, 0, (size_t)n * sizeof(fe_prog_t));

    const vv_vae_weights_t* w = fe->w[0] ? fe->w[0] : fe->w[1];
    vv_status_t first_err = VV_OK;

    vv_mutex_lock(&fe->lock);
    fe->n_jobs += (uint64_t)n;
    for (int j = 0; j < n; j++) {
        jobs[j]->status = VV_OK;
        jobs[j]->n_frames = 0;
        if (!jobs[j]->audio && jobs[j]->n_samples > 0) {
            finish_job(fe, jobs[j], &pg[j], VV_ERR_NULL_PTR);
            first_err = VV_ERR_NULL_PTR;
        } else if (jobs[j]->stream && jobs[j]->stream->fe != fe) {
            finish_job(fe, jobs[j], &pg[j], VV_ERR_INVALID_ARG);
            first_err = VV_ERR_INVALID_ARG;
        }
    }
    ...
    for (;;) {
        ...
        int np = 0;
        int64_t room = fe->p.max_samples;
        for (int j = 0; j < n && np < fe->p.max_items; j++) {
            if (pg[j].finished) continue;
            vv_frontend_job_t* jb = jobs[j];
            const int64_t rem = jb->n_samples - pg[j].done;
            if (rem <= 0) {
                /* Nothing new. Only the end of a stream still has work: the
                   right-edge padding of every layer. */
                if (!jb->stream || !jb->is_final || pg[j].started) {
                    finish_job(fe, jb, &pg[j], VV_OK);
                    continue;
                }
            }
            const int64_t take = rem < room ? rem : room;
            if (rem > 0 && take <= 0) break;            /* launch is full */
            if (take < rem && !jb->stream) {
                if (np > 0) break;                      /* alone, next time */
                VV_LOG_E("frontend: a stateless window of %lld samples "
                         "exceeds the %lld of one launch",
                         (long long)rem, (long long)fe->p.max_samples);
                finish_job(fe, jb, &pg[j], VV_ERR_OVERFLOW);
                first_err = VV_ERR_OVERFLOW;
                continue;
            }
            fe_piece_t* p = &pc[np++];
            p->job = jb;
            ...
            {
                const int want_skip = jb->skip_frames - pg[j].produced;
                p->skip = want_skip <= 0 ? 0
                        : (want_skip > p->frames ? p->frames : want_skip);
            }
            p->written0 = pg[j].written;
            room -= take;
            pg[j].started = true;
        }
        if (np == 0) break;
        ...
        for (int i = 0; i < np; i++) {
            int j = 0;
            while (jobs[j] != pc[i].job) j++;
            if (s != VV_OK) {
                finish_job(fe, jobs[j], &pg[j], s);
                continue;
            }
            pg[j].done += pc[i].len;
            pg[j].produced += pc[i].frames;
            pg[j].written += pc[i].frames - pc[i].skip;
            if (pg[j].done >= jobs[j]->n_samples)
                finish_job(fe, jobs[j], &pg[j], VV_OK);
        }
        if (s != VV_OK) {
            first_err = s;
            for (int j = 0; j < n; j++)
                if (!pg[j].finished) finish_job(fe, jobs[j], &pg[j], s);
            break;
        }
    }
    for (int j = 0; j < n; j++)
        if (!pg[j].finished) finish_job(fe, jobs[j], &pg[j], VV_OK);
    vv_mutex_unlock(&fe->lock);
    if (pg != stack_pg) vv_free(pg);
    return first_err;
}
...
static VV_THREAD_RET service_main(void* arg) {
    ...
    for (;;) {
        while (!fe->q_head && !fe->svc_stop) vv_cond_wait(&fe->q_cond, &fe->q_lock);
        if (!fe->q_head && fe->svc_stop) break;

        /* A short gather window: whatever else arrives in it shares the
           launches. Skipped when the batch is already full. */
        {
            int queued = 0;
            int64_t samples = 0;
            for (fe_req_t* r = fe->q_head; r; r = r->next) {
                queued++;
                samples += r->job->n_samples;
            }
            if (fe->p.gather_us > 0 && queued < fe->p.max_items &&
                samples < fe->p.max_samples && !fe->svc_stop) {
                vv_mutex_unlock(&fe->q_lock);
                vv_sleep_ms(fe->p.gather_us >= 1000 ? fe->p.gather_us / 1000 : 1);
                vv_mutex_lock(&fe->q_lock);
            }
        }

        fe_req_t* batch[VV_VAE_MAX_ITEMS];
        vv_frontend_job_t* jobs[VV_VAE_MAX_ITEMS];
        int n = 0;
        while (fe->q_head && n < fe->p.max_items) {
        ...
        vv_mutex_lock(&fe->q_lock);
        for (int i = 0; i < n; i++) batch[i]->done = true;
        vv_cond_broadcast(&fe->q_done);
    }
    vv_mutex_unlock(&fe->q_lock);
    VV_THREAD_RETURN;
}
...
vv_status_t vv_frontend_service_start(vv_frontend_t* fe, int device) {
    ...
    fe->svc_on = true;
    VV_LOG_I("frontend: batching service on gpu %d (gather %d us, up to %d "
             "jobs / %.0f s per launch)", device, fe->p.gather_us,
             fe->p.max_items, (double)fe->p.max_samples / 24000.0);
    return VV_OK;
}
