...
* `POST /v1/chat/completions` — та же модель как LLM, без аудио
  (`vv_inference_generate` в `pipeline.c`): сообщения рендерятся в ChatML,
  `stream: true` шлёт `chat.completion.chunk` (дельты режутся по границе
  UTF-8), `temperature`/`top_p`/`top_k`/`seed`/`stop`/`max_completion_tokens`
  работают, по умолчанию greedy. Сэмплирование тянет логиты на хост
  (`vv_sample_logits_f32`), greedy остаётся на device-argmax. Ответ
  заканчивается на `<|im_end|>` **и на `<|im_start|>`** — иначе дообученная
  на одной задаче модель открывает новый ход и пишет диалог за обе стороны.
  Генерация занимает слот, как транскрипция. 3090, AWQ: 135 tok/s.
  Качество ответов — какое есть у ASR-чекпоинта: он охотно транскрибирует
  сам вопрос. В ответах транскрипции есть `usage`
  (`{"type":"duration","seconds":N}`) — по нему шлюз считает потребление.
...
* **Спекулятивный декод DFlash 2** (`--draft <dir>`, `docs/DFLASH.md`,
  `include/vibevoice/spec.h`, `src/inference/spec.c`, `src/cuda/dflash.cu`):
  драфтер (5 слоёв Qwen3, KV-инъекция слоёв 1/7/13/19/25 цели,
  двухтаповая динамическая свёртка, селектор кандидатов) предлагает блок из
  8 токенов за проход, модель проверяет `--draft-block` строк одним
  проходом и оставляет совпавший префикс. `--draft-check exact` (по
  умолчанию): каждая строка считается арифметикой однотокенного шага,
  транскрипт побайтно тот же, что без драфтера. W4A16-проекции шага и
  проверки — одно тензорное ядро `vv_w4a16_mv_dev` (веса — операнд A mma,
  строки x — столбцы B, K режется на 16 фиксированных срезов, дерево
  сложения): биты строки не зависят от M, слияния проекций и формы запуска,
  8 строк стоят ~1.1 шага. **Из-за этого обычный W4A16-декод (AWQ, GPTQ,
  `--quant int4`) сменил биты относительно 0.5.1** (FP32-суммы в mma вместо
  FP16-цепочек), хотя транскрипты AWQ 7B на jfk/test30/test120/32 мин и
  Streaming-1.5B на jfk/test120 посимвольно те же; обычный декод 7B на 1–2%
  медленнее (GEMV слоя 1.5B — на 10% быстрее); `VV_W4A16_MV=0` — старый GEMV.
  Внимание строк: fa2 — слой сетки на строку (скалярный decode упирается в
  цепочку редукций, общий проход по KV не помог); flashinfer — 2–3 строки в
  одном 16-строчном фрагменте (24K позиций, 8 строк: 1.64 шага внимания).
  Поэтому с драфтером `--attn auto` = flashinfer (AWQ 7B, 32 мин: exact
  1.06× на fa2 против 1.47× на flashinfer), транскрипт — как у
  `--attn flashinfer` без драфтера; `--attn fa2 --draft` оставляет fa2.
  `--draft-check fast`: те же строки через flashinfer и проекции префилла.
  Цикл (драфт, проверка, приём, контекст драфтера) читает позиции с
  устройства и идёт одним CUDA-графом (перезахват раз в 1024 позиции;
  Windows/WDDM 3070: 2.76× против 1.79× без графа, Linux 3090: +4%).
  Драфтер INT4 по умолчанию, может храниться в INT4 (AWQ,
  `tools/dflash/awq_drafter.py`). Контроллер: блоки, пока они быстрее шага.
  One-shot, стриминг-сессии, chat (greedy), `serve`. Metal/CPU/`--split-mode
  layer` — отказ с предупреждением. 3090, exact, транскрипты побайтно те
  же: ASR-7B AWQ (`Ar4ikov/VibeVoice-ASR-DFlash2-Drafter`) против fa2 без
  драфтера — jfk 3.85×, test120 3.82×, 32 мин 2.73×, 20 клипов 3.84×
  (против flashinfer), BF16 7B — test120 2.73×, 32 мин 2.36×;
  Streaming-7B — test120 2.59×, 32 мин 2.10×, клипы 2.45×;
  Streaming-1.5B — test120 2.50×, 32 мин 2.13×, клипы 2.12×. На Hub —
  драфтеры (BF16 и `-AWQ-W4A16-ASYM`) и сборки `<AWQ-модель>-DFlash2`
  (и BF16-чекпоинты Microsoft — `Ar4ikov/VibeVoice-ASR-DFlash2` и т.д.) с
  драфтером в `drafter/`, который рантайм берёт без `--draft`. Проверка 8
  строк 7B — ~1.3 шага, так что ускорение ≈ токены на блок / 1.3; драфтеры
  упирались в данные (v4 7B: 5.4 токена на блок на train против 3.7 на
  eval; v5 на 2.8× аудио и с ограничением якорей в петлях — 5.27 на eval).
  NF4/INT8 проверяют exact по строке и не окупаются — там драфтер
  отклоняется, `--draft-check fast` идёт через small-M ядро (`gemm_skinny.cu`)
  с 2 строк. Драфтер BitNet не учится (depth-1 0.24 против 0.80), не
  опубликован.
...
* В контейнере номера карт свои, с нуля. GPUStack 2.2.2 монтирует
  выданные карты по UUID (`NVIDIA_VISIBLE_DEVICES`) и **при этом** ставит
  `CUDA_VISIBLE_DEVICES` с хостовыми индексами: реплика, получившая хостовую
  карту 1, видит `/dev/nvidia1` как устройство 0, переменная просит
  устройство 1 — и CUDA не находит ни одной карты (модель молча уезжала на
  CPU). С 0.5.0 список сверяется с узлами `/dev/nvidiaN` и переводится в
  локальную нумерацию до первого вызова CUDA (`vv_cuda_visible_map`,
  `vv_cuda_fix_visible_devices`, issue #41); на хосте, где нумерации
  совпадают, ничего не меняется, `VV_KEEP_CUDA_VISIBLE_DEVICES=1` отключает.
  Вторая ловушка того же планировщика: без ручного режима (GPU Selector +
  GPUs per Replica) кастомному бэкенду достаётся **одна** карта, `--gpus all`
  больше не попросит; при этом `/dev/nvidia*` смонтированы все. Если узлов в
  `/dev` больше, чем в `CUDA_VISIBLE_DEVICES`, лог это говорит. Правка модели
  в GPUStack 2.2.2 не перекатывает живую реплику — её надо удалить.
...
* **Metal — вторая реализация `device.h`** (`src/metal/`, `docs/METAL.md`):
  весь конвейер на GPU Apple Silicon, код выше шва тот же. Все форматы весов
  и KV, три бэкенда внимания, захват графов, страничный кэш. Xcode не нужен —
  MSL лежит в бинарнике исходником, Metal-фреймворк компилирует его при
  первом запуске. Остаются медленными: Conv-VAE энкодер (2.2 c на 30 c
  аудио), тернарный GEMV BitNet (8–17 GB/s из 96), `vv_skinny_linear_dev`
  отказывается (9..64 строки идут тайловым GEMM), целочисленного матричного
  блока у Apple нет (`VV_I8_PATH_MMA` отказывается), мульти-GPU — одно
  устройство.
* **Общая память считается памятью машины.** Где GPU делит RAM с хостом
  (`vv_dev_host_shares_memory`), хостовая копия выгруженного тензора
  освобождается (embedding, head, FP32-веса энкодеров — 4.7 GB у 7B), а
  `vv_gpu_budget_reclaim` возвращает эти байты в бюджет: перенос на
  устройство — это memcpy, а не вторая копия. «Свободно» у устройства
  ограничено ещё и тем, что реально может дать ОС, иначе окно KV обещает
  32K и машина уходит в своп.
...
| Параметр | Значение |
|----------|----------|
| ОС | Windows 11 x64, Linux x64, macOS 14+ (Apple Silicon — Metal) |
...
### Почему не Neural Engine (Apple Silicon)

Замерено на M4 (`docs/ANE.md`, воспроизводится `tools/ane_probe/`). ANE
доступен только через Core ML: веса запекаются в скомпилированную модель,
граф фиксирован, компиляция 8–65 с. На FP16-матмуле prefill-формы
(256×3584×18944) ANE даёт **14.9 TFLOP/s** — в 7 раз быстрее наших
Metal-ядер. Но: с int4-весами Core ML вообще перестаёт его использовать
(0 из 3 операций на ANE, 2.2 TFLOP/s на GPU), а 7B в FP16 — это 15 GB на
16-гигабайтной машине. Decode упирается в память: ANE читает на 62 GB/s
против 94 GB/s у нашего NF4-GEMV, который к тому же читает 4 бита, а не 16
(0.385 мс против 2.18). Граф энкодера ANE берёт целиком, но выполняет
**втрое медленнее GPU** — у него на входе 32 канала × 720 000 позиций, а он
построен под обратное; 30-секундный вход не компилируется вовсе
(`ANECCompile() FAILED`).

Побочная находка тех же замеров: Core ML на GPU прогоняет наш же граф
энкодера за 10.2 мс на секунду аудио против ~36 мс у нашего энкодера на
устройство — разница в FP32-свёртках, которые мы держим ради побитовой
воспроизводимости. Это запас на нашей стороне, а не у ANE. И CPU-путь
Core ML достаёт 2.93 TFLOP/s через AMX, которого NEON-ядра `src/cpu/` не
используют.
