...
| Метрика | Факт |
|---|---|
| Загрузка модели | **4.0 c** (NF4), 5.5 c (AWQ, включая репак; ещё ~0.9 c на GPU-раскладку W4A16); потоки = физические ядра |
...
* Веса: NF4 (bitsandbytes), AWQ, GPTQ. AWQ/GPTQ перепаковываются при
  ...
  `src/cuda/w4a16.cu`: GEMV с LOP3/HFMA2 и Marlin-подобный GEMM на tensor
  cores для M > 1 без 136 MB FP16-скретча. Деквант — `(q − z)·s`, как в
  эталоне. GPTQ act-order (`desc_act`) отклоняется, `VV_INT4G_LEGACY=1` —
  старый путь.
