...
* `--gpus 0,1|all` выбирает устройства, `--gpu-memory 80%|18GiB|8192M|байты`
  ...
  Шардирование одной модели по картам — ещё нет (issue #7).
...
* **Шардирование модели по нескольким GPU** (issue #7). Выбор устройств,
  лимиты памяти и реплики на устройство есть; pipeline-параллелизм по слоям
  и распределённый KV-кэш — нет.
